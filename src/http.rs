...
pub async fn start_proxy(config: ParsedConfig) -> Result<JoinHandle<Result<(), anyhow::Error>>> {
    let l = TcpListener::bind(&config.general.http_listen).await?;
            ...
            Some((protocol, host, port, auth)) => {
                ...
                Ok(match protocol {
                    Protocol::Socks5 => tokio::spawn(async move {
                        loop {
                            let (stream, socket_addr) = l.accept().await.unwrap();
...
{
    let proxy = netway::tunnel::ProxyServer::new(dialer);
    match proxy.serve(socket, &auth).await {
        Ok((a_2_b, b_2_a)) => {
            log::info!(
                "finish http proxy from {}, up: {}, down: {}, ",
                &dst_addr,
                a_2_b,
                b_2_a
            );
        ...
        Err(e) => match e {
            ...
            _ => {
                log::error!("finish http proxy from {}, {}", &dst_addr, &e);
                Err(e)
            }
        },
    }
