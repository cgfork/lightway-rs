...
pin_project! {
    pub struct Duplex<I, O> {
        half_in: HalfDuplex<I>,
        half_out: HalfDuplex<O>,
    }
}
...
{
    pub fn new(in_io: I, out_io: O) -> Self {
        Duplex {
...
impl<I, O> Future for Duplex<I, O>
where
    I: AsyncRead + AsyncWrite + Unpin,
    O: AsyncRead + AsyncWrite + Unpin,
{
    ...
    T: AsyncRead + Unpin,
{
    ...
    fn copy_into<U: AsyncWrite + Unpin>(
        ...
        // the copy loop.
        if dst.is_shutdown {
            trace!("{} already shutdown", self.direction);
            ...
            match self.poll_buffer(cx)? {
                ...
                Poll::Ready(Buffered::NotEmpty) | Poll::Ready(Buffered::Read(_)) => {
                    ...
                    match self.drain_into(dst, cx)? {
                        ...
                        Drained::BufferEmpty => {
                            error!("{} attempts to write from an empty buffer", self.direction);
                ...
                Poll::Ready(Buffered::Eof) => {
                    trace!("{} shutting down", self.direction);
    ...
    /// will be read.
    fn poll_buffer(&mut self, cx: &mut Context<'_>) -> Poll<io::Result<Buffered>> {
        ...
        if let Some(buf) = self.buf.as_mut() {
            ...
            buf.reset();
            trace!("{} reading", self.direction);
            let sz = ready!(poll_read_buf(Pin::new(&mut self.io), cx, buf))?;
            trace!("{} read {}B", self.direction, sz);
